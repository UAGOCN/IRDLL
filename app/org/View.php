...
class View {
    ...
    private $system_replace = array(
        ...
        '~\{(include_once|require_once|include|require)\s*\(\s*(.+?)\s*\)\s*\s*\}~i' => '<?php \$this->_include($2, __FILE__) ?>',
        # {include('inc/top.php')}
    ...
    public function render($file, $data = null) {
        ...
        if (!$this->isCached($tmpPath)) {
            ...
            @file_put_contents($tmpPath, trim($tpl), LOCK_EX);
        }
    ...
    public function _include($file, $data = null) {
        return $this->render($file, $data = null);
    }
