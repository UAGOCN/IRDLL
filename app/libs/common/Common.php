...
class Common extends app\Engine {
    ...
    public function getTimeOut() {
        return trim(ceil($this->getDatas()['timeout']));
    }
    ...
    public function getToken($srt = null) {
        if(!empty($srt)||empty($this->sess()->getSession('token'))||time()>unserialize($this->sess()->getSession('token'))[3]) {
            $getToken = $this->getTokenID('GET');
            $postToken = $this->getTokenID('POST');
            $token = md5($getToken.uniqid().time().$postToken);
            $this->sess()->setSession('token',serialize(array($token, $getToken, $postToken,time()+$this->fun()->getTimeOut())));
            return $token;
        } else {
