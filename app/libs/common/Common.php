...
class Common extends app\Engine {
    ...
    public function getTokenID($value = 'GET') {
        return md5(md5($value).md5(htmlspecialchars($this->request()->url, ENT_QUOTES, 'UTF-8')).md5($this->request()->scheme).md5($this->request()->user_agent).md5($this->request()->cookies[$this->fun()->getSessionName()]).md5($this->request()->accept).unserialize($this->sess()->getSession('key'))[0]);
    }
