    ...
    public static function forgot_password() {
        ...
        if(Api::request()->method==='POST') {
            ...
            if(isset(Api::request()->data['__hash__'])&&md5($toke[0])===md5(Api::request()->data['__hash__'])&&$toke[1]===(Api::fun()->getTokenID('GET'))&&$toke[2]===(Api::fun()->getTokenID('POST'))) {
                ...
                $outEmail = Api::fun()->getDB()->field('user_name')->where(array('user_email'=>array($inEmail)))->limit(1)->select('user');
                if(!empty($outEmail[0])) {
                    Api::fun()->getDB()->where(array('user_name'=>$outEmail[0]['user_name']))->update('user',array('user_session'=>md5(Api::request()->cookies[Api::fun()->getSessionName()]),'user_del'=>2,'user_ip'=>trim(Api::fun()->getSrt('ip',Api::request()->ip)),'user_logintime'=>time()));
                    Api::fun()->getSMTP($inEmail, $outEmail[0]['user_name'], '['.Api::fun()->getTitle().'] 重置密码说明', Api::fun()->getForgotMB($outEmail[0]['user_name'], $inEmail, md5(Api::request()->cookies[Api::fun()->getSessionName()])));
                    header('Location: /login');
                    exit();
                } else {
                    header('Location: /forgot-password');
